...
class ActualizarAlimentoScreen (val alimento: DatosAlimento): Screen {
    ...
    override fun Content() {
        ...
        var cantidad by remember { mutableStateOf(alimento.cantidadMedida.toString()) }
        var marca by remember {mutableStateOf(alimento.marca)}
        var medida by remember { mutableStateOf(alimento.medida) }
        var expandedMedida by remember { mutableStateOf(false) }
        val opcionesMedida = listOf("g", "ml")

        var calorias by remember { mutableStateOf(alimento.calorias.toString()) }
        var proteinas by remember { mutableStateOf(alimento.proteinas.toString()) }
        var carbohidratos by remember { mutableStateOf(alimento.carbohidratos.toString()) }
        var grasas by remember { mutableStateOf(alimento.grasas.toString()) }
        ...
        Scaffold(
            ...
            topBar = {
                CenterAlignedTopAppBar(
                    title = { Text("Actualizar Alimento", color = BlancoPuro) },
                    navigationIcon = {
            ...
            floatingActionButton = {
                FloatingActionButton(
                    onClick = {
                        ...
                        // Datos actualizados del alimento
                        val alimentoActualizado = hashMapOf(
                            "id" to alimento.id,
                        ...
                        db.collection("MisAlimentos")
                            ...
                            .update(alimentoActualizado as Map<String, Any>)
                            .addOnSuccessListener {
                                Toast.makeText(context, "Alimento actualizado correctamente", Toast.LENGTH_SHORT).show()
                                navigator.popUntilRoot()
                            }.addOnFailureListener {
                                Toast.makeText(context, "Error al actualizar el alimento", Toast.LENGTH_SHORT).show()
                            }
                    },
            ...
            Column(
                ...
                Text("Información del alimento", color = GrisSuave, fontWeight = FontWeight.SemiBold)
                ...
                // Fila de cantidad + medida (Dropdown)
                Row(
                    modifier = Modifier.fillMaxWidth(),
                    verticalAlignment = Alignment.CenterVertically,
                    ...
                    // Selector de medida (Lista Desplegable)
